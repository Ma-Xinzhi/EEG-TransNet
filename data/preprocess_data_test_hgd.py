...
from hgd_signal_utils import highpass_cnt, mne_apply, exponential_running_standardize

data_path = 'raw_data/test'
save_path = 'process_data/test'
...
for sub_id in range(1,15):
    ...
    test_data, test_label = create_data_label_from_raw_mne(raw_mne, marker_def, time_window)
    test_data = test_data[clean_trial_mask]
    test_label = test_label[clean_trial_mask]
    print('Data shape', test_data.shape)
    print('Label shape', test_label.shape)

    np.save(os.path.join(save_path, str(sub_id)+'_data.npy'), test_data)
    np.save(os.path.join(save_path, str(sub_id)+'_label.npy'), test_label)
